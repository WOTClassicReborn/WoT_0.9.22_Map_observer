...
class PlayerAvatar(PlayerAvatar):
    def __init__(self):
        if g_instance.isStarted:
            ...
            self.spaceMappingID = BigWorld.addSpaceGeometryMapping(
                self.spaceID, None, 'spaces/%s' % g_instance.spaceName
            )
            BigWorld.cameraSpaceID(self.spaceID)
            BigWorld.wg_setSpaceItemsVisibilityMask(
                self.spaceID, g_instance.arenaVisibilityMask
            )

            if Account.g_accountRepository is None:
                Account.g_accountRepository = Account._AccountRepository(
    ...
    def onBecomePlayer(self):
        ...
        if g_instance.isStarted:
            try:
                # Берём танк из g_instance
                vehicleTag = g_instance.selectedVehicleTag
                LOG_DEBUG('Spawning vehicle from hangar: %s' % vehicleTag)
                self.addBotToArena(vehicleTag, self.team)
            except Exception:
                LOG_DEBUG('ERROR in addBotToArena')
    ...
    def vehicle_onEnterWorld(self, vehicle):
        if g_instance.isStarted:
            if self.playerVehicleID == 0:
                ...
                LOG_DEBUG('Player vehicle set vehicleID=%s' % vehicle.id)
    ...
    def __onSetOwnVehicleAuxPhysicsData(self, prev):
        if g_instance.isStarted:
            ...
            if vehicle is not None and vehicle.isStarted:
                y, p, r, leftScroll, rightScroll, normalisedRPM = \
                    WoT.unpackAuxVehiclePhysicsData(self.ownVehicleAuxPhysicsData)
